...
module.exports = {
  name: "language",
  description: "Beállíthatod a bot nyelvét.",
    ...
    try {
      ...
      let buttons = new ActionRowBuilder().addComponents(
        ...
        new ButtonBuilder()
          ...
          .setEmoji('🇭🇺'),
        new ButtonBuilder()
          .setLabel("Magyar(Egyedi)")
          .setCustomId('hUWUngarian')
          .setStyle(ButtonStyle.Secondary)
          .setEmoji('(*＞ω＜*)♡'),
      )
        ...
        col.on('collect', async (button) => {
          ...
          switch (button.customId) {
            ...
            case 'hu':
              ...
              await interaction?.editReply({ content: `A bot nyelve sikeresen átállítva magyarra! :flag_hu:`, embeds: [], components: [], ephemeral: true }).catch(e => { })
              await button?.deferUpdate().catch(e => { })
              await col?.stop()
              break
            ...
            case 'hu2':
              await db?.musicbot?.updateOne({ guildID: interaction?.guild?.id }, {
                $set: {
                  language: 'hUWUngarian'
                }
              }, { upsert: true }).catch(e => { })
              await interaction?.editReply({ content: `A bot nyelve sikeresen átállítva Egyedi magyarra ^^! :flag_hu:`, embeds: [], components: [], ephemeral: true }).catch(e => { })
              await button?.deferUpdate().catch(e => { })
              await col?.stop()
              break
        ...
        col.on('end', async (button, reason) => {
          if (reason === 'time') {
            ...
            embed = new EmbedBuilder()
              .setColor(client.config.embedColor)
              .setTitle("Lejárt az idő, próbáld újra.")
              .setTimestamp()
              .setFooter({ text: `Muzsikabot by K,M,G` })
